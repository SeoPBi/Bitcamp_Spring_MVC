...
public class MVCController03 {
    ...
    public String naver() {
        ...
        // 마치 a태그를 클릭하는 것처럼 자동으로 페이즈를 보낼 주소로 이동시킵니다.
        // 이과정에서 포워딩과는 달리 model.addAtrribute()처럼 자료를 첨부해 이동 시킬수는 없습니다.
        // 하지만 get방식으로 이동하는 만큼 쿼리스프링을 붙여서 보내는 것은 가능합니다.
        return "redirect:https://www.naver.com";
    }

    @RequestMapping("/goFoodform")
    public String goFoodForm(Model model) {
        // 리다이렉션은 model.addAtrribute()를 통한 데이터 전송이 불가능합니다.
        model.addAttribute("title", "음식주문 키오스크"); // test라는 이름으로 "test" 문자열 삭제
        // http://localhost:8080//chap03/foodform 이라는 주소로 이동(라다이렉트이므로 파일지정이 아님)
