...
public class MVCController01 {
    ...
    // 특정 주소로 접속했을 때 처리할 로직을 작성합니다.
    @RequestMapping("/hello") // localhost:8080이 기본 주소이고, 뒤에 /hello를 붙여 접속시 실행
    public String hello() {
        System.out.println("/hello 접속 감지!!!!!!");
        // 어떤 .jsp파일과 연동할지 return구문에 문자열로 파일 이름을 적어줌
    ...
    // 스프링 3버전까지 쓰던 url매핑 방법                                    배열로 나열시 2개 이상 메서드 허용 가능
    ...
    @PostMapping("/postreq") // localhost:8080/getpost 접속 허용
    ...
    /*
        요청 파리미터 읽어오기(Query String = GET 방식, RequestBody = POST 방식)
        1. HttpSevletReuest 사용하기(원시적인 JSP에서 쓰는 방법)
    ...
    public String major(String stu,
                        @RequestParam(defaultValue = "0.0") double gpa, // 파라미터 입력 안될시 기본값 지정도 가능
    ...
    /*
        ...
        외부 클래스의 내부 멤버 변수명이 전부 매핑되어 주입됨.
     */
    ...
    public String notebook(NotebookDTO notebook) {
        System.out.println("파라미터로 전달받은 NotobookDTO 내부 : " + notebook);
        return "";
    }

    /*
        여러분들이 임의의 객체를 만들어서, 멤버 변수명으로 파라미터를 전달 받아
        콘솔창에 찍어주세요.
    ...
    public String keyboard(KeyboardDTO kerboard) {
        System.out.println("파라미터로 전달받은 KeyboardDTD 내부 : " + kerboard);
        return "";
    }
    ...
    public String board(@PathVariable int bNo) { // 왼쪽에 @PathVariable을 붙여 줍니다.
    ...
    @RequestMapping("/member/{mNo}/{mName}")
    public String member(@PathVariable int mNo, @PathVariable String mName) {
        System.out.println("경로로 받아온 mNo의 값 : " + mNo);
        System.out.println("경로로 받아온 회원이름 : " + mName);
        return "";
    }
